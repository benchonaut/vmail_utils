...
echo "SELECT * FROM vmail.\`domains\` WHERE domain='"$domain"'" | mysql -u vmail -p"$vmailpass"|grep -q "$domain" || ( echo DOMAIN NOT REGISTERED IN SYSTEM ; exit 3 ) && 
	echo "SELECT * FROM vmail.\`aliases\` WHERE source='@"$domain"' AND destination='"$destination"'"  | mysql -u vmail -p"$vmailpass"|grep "$usertogen"'@'"$domain"|grep "$destination"  && echo DIRECT ALIAS ALREADY THERE ||
